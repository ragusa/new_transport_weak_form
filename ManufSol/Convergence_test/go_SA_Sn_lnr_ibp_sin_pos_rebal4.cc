...
# define Qh 1.0      //homogenous solution
# define mesh_refinement 0
  ...
  for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    for(unsigned int i_dof=0; i_dof<dofs_per_cell; i_dof++)
    {
      ...
      k_abs_vertex(local_dof_indices[i_dof]) = cos(M_PI/2.0*dof_support_point(0))*
    		                                       ...
    		                                       /*std::fabs(2.0+sin(M_PI/2.0*dof_support_point(0)))
    		                                            *(2.0+sin(M_PI/2.0*dof_support_point(1)))
    		                                            *(2.0+sin(M_PI/2.0*dof_support_point(2)))*0.001; */
    }
  }
...
{ // here we just solve the system of equation thanks to CG we use a SSOR preconditioner 
 ...
 std::cout<<"L2(RHS) = "<<system_rhs.l2_norm()<<std::endl;
...
class SN // here we define the SN class which contains the Parameters class
{
 ...
 private :
  ...
  void compute_rebalance(unsigned int group);        //compute rebalance factor with dirichlet boundary absorption removed from the source
  void compute_L2_norm_error(unsigned int group);     //compute the L2 norm of the error in solution
  void renormalization(unsigned int group);                 //renormalize the solution to force conservation
  ...
  unsigned int    n_groups;
  std::vector<double> rebalance;    //artificial coefficient to make energy balance
...
//    std::cout<<"st_t_grad = "<<st_t_grad[q_point]<<std::endl;
//    std::cout<<"B_grad = "<<B_grad[q_point]<<std::endl;
  }
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template<int dim>
void SN<dim>::compute_rebalance(unsigned int group)
{
	double source = 0.0;  //total particle gain
	double sink = 0.0;    //total particle loss
	double diri_contribution = 0.0; //particle loss due to absorption assoicated with the Dirichlet boundary nodes
	double conservation = 0.0;  //conservation = (source-sink)/source
         ...
         for (unsigned int i=0; i<dofs_per_cell; ++i)
         {
           if(fe.has_support_on_face(i, face))
           {
             ...
             { 
          	   if(Omega[m]* fe_face_values_dummy.normal_vector(i_face_dof) <= 0)
          	   {
          	   	   double diri_value = 0.0;
          	   	   
          	       sn_group[group]->solution_moment[m](local_dof_indices[i]) = diri_value;  //T4_vertex represents the Manufacturered solution here!!!
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
	...
	//substract sink_diri_abs from both the source and the sink
	diri_contribution = tot_sink - sink;
	source = tot_source - diri_contribution;
	
	
  std::cout<<"Reduced sink = "<<sink<<std::endl;
  std::cout<<"Reduced source = "<<source<<std::endl;
  
  rebalance[group] = sink/source;
  std::cout<<"Rebalance ratio ["<<group<<"] = "<<rebalance[group]<<std::endl;
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>  
...
//	error_solution[group].reinit(sn_group[group]->dof_handler.n_dofs());
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    {   	
      ...
      double q_point_solution = cos(M_PI/2.0*fe_values.quadrature_point(q_point)(0)) *
    	                       cos(M_PI/2.0*fe_values.quadrature_point(q_point)(1)) *
    	                       cos(M_PI/2.0*fe_values.quadrature_point(q_point)(2))
    	                       + Qh;
    	                       
      double q_point_error = phi_values[q_point] - q_point_solution;
    	                       
    	                       
    	l2_solution += pow(q_point_solution, 2.0)*fe_values.JxW (q_point);
  	  l2_error += pow(q_point_error, 2.0)*fe_values.JxW (q_point);
    }
  }
  
  std::cout<<"=============================================="<<std::endl;
  std::cout<<"L2(u - u_h)["<<group<<"] = "<<sqrt(l2_error)<<std::endl;
  std::cout<<"L2(u - u_h)/L2(u)["<<group<<"] = "<<sqrt(l2_error)/sqrt(l2_solution)<<std::endl;
  std::cout<<"=============================================="<<std::endl;
}	
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>
void SN<dim>::renormalization(unsigned int group)
{
    for(unsigned int m=0; m< n_Omega; m++)
    {  
    	sn_group[group]->solution_moment[m] /= rebalance[group];
    }
