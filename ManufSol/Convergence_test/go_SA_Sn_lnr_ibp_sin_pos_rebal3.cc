...
//	}
  ...
  for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    for(unsigned int i_dof=0; i_dof<dofs_per_cell; i_dof++)
    {
      ...
      k_abs_vertex(local_dof_indices[i_dof]) = 0.0001*cos(M_PI/2.0*dof_support_point(0))*
    		                                                 cos(M_PI/2.0*dof_support_point(1))*
    		                                                 cos(M_PI/2.0*dof_support_point(2));
    }
  }
   	   ...
   	   if (cell->at_boundary(face) /*&& (parameters.boundary_conditions[cell->face(face)->boundary_indicator()] == 2)*/)
       {
         for (unsigned int i=0; i<dofs_per_cell; ++i)
         {
           if(fe.has_support_on_face(i, face))
           {
        	   ...
        	   {
        	   	   double diri_value = 10.0/(4.0*M_PI);
...
void SN<dim>::compute_rebalance(unsigned int group)
{
	double source = 0.0;  //total particle gain
	double sink = 0.0;    //total particle loss
	double sink_diri_abs = 0.0; //particle loss due to absorption assoicated with the Dirichlet boundary nodes
	double conservation = 0.0;  //conservation = (source-sink)/source
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
  		    ...
  		    for(unsigned int angle = 0 ; angle < n_Omega; angle++)
          {
           ...
           for (unsigned int i=0; i<dofs_per_cell; ++i)
           {
              if(fe.has_support_on_face(i, face))
              {
              	 ...
              	 for(unsigned int angle = 0 ; angle < n_Omega; angle++)
                 { 		
               	   if(Omega[angle]* fe_face_values_dummy.normal_vector(i_face_dof) <= 0)
               	   {//std::cout<<local_dof_indices[i]<<" ";
               	   	 for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
  		               {
  			               sink_diri_abs += sn_group[group]->solution_moment[angle](local_dof_indices[i])
  			                                *fe_values.shape_value(i,q_point)
  			                                *sa_t[q_point]
  		                	                *fe_values.JxW(q_point)
  		                	                *(dim==2?2.0:1.0)*wt[angle];		    //absorption rate due to Dirichlet nodes
  		               }
	...
	source = source - sink_diri_abs;
	sink   = sink - sink_diri_abs;
	
  std::cout<<"sink = "<<sink<<std::endl;
  std::cout<<"source = "<<source<<std::endl;
	conservation = (source-sink)/source;
  std::cout<<"Particle Conservation : "<<conservation<<std::endl;
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    {   	
			double q_point_error = phi_values[q_point] - 
			                       cos(M_PI/2.0*fe_values.quadrature_point(q_point)(0)) *
