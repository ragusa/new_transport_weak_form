...
//	}
  ...
  for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    for(unsigned int i_dof=0; i_dof<dofs_per_cell; i_dof++)
    {
    	...
    	T4_vertex(local_dof_indices[i_dof]) = cos(M_PI/2.0*dof_support_point(0))*
    		                                                 cos(M_PI/2.0*dof_support_point(1))*
    		                                                 cos(M_PI/2.0*dof_support_point(2));
    	
      k_abs_vertex(local_dof_indices[i_dof]) = 0.001*cos(M_PI/2.0*dof_support_point(0))*
    		                                                 cos(M_PI/2.0*dof_support_point(1))*
    		                                                 cos(M_PI/2.0*dof_support_point(2));
    }
  }
 ...
 for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  ...
  if (boundary_exist != boundary_conditions.end()) 
  {
                       ...
                       // condition we go to the if
    {
      ...
      {
      	unsigned int side = cell->face(face)->boundary_indicator();  //Boundary ID is like 10, 20, 30, 40...
...
//  grid_in.read_ucd(in);
   ...
   const Point<3> upper_right = Point<3> (-1.0, -1.0, -1.0);
   
  GridGenerator::hyper_rectangle (t, bottom_left, upper_right, true);
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
			...
			for(unsigned int angle = 0 ; angle < n_Omega; angle++)
      {
		...
		for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face) 		// we make a loop over all the face, if we have a face where we have a boundary	 
  		if (cell->at_boundary(face)) 
  	  {							
  	  	unsigned int side = cell->face(face)->boundary_indicator();																		// condition we go to the if
          ...
          fe_face_values.get_function_values(material_data.get_T4_vertex(), T4_face);
  				  ...
  				  for(unsigned int angle = 0 ; angle < n_Omega; angle++)
            {
              ...
              fe_face_values.get_function_values(psi_plus_m,face_psi_values);	
       		    ...
       		    if(Omega[angle]*fe_face_values.normal_vector(q_point) > 0.0)  //check if current Omega is out-going direction
          	  {  
                sink +=  std::fabs( Omega[angle] * fe_face_values.normal_vector(q_point)  ) *
...
//    {
//    	unsigned int i_dof_g = local_dof_indices[i];
//    	error_solution[group](i_dof_g) = phi[group](i_dof_g) - 
//    	                                 cos(M_PI/2.0*fe_values_dummy.quadrature_point(i)(0)) *
//    	                                 cos(M_PI/2.0*fe_values_dummy.quadrature_point(i)(1)) *
//    	                                 cos(M_PI/2.0*fe_values_dummy.quadrature_point(i)(2));
//    }
//  }
//  
//  std::cout<<"L2(u - u_h)["<<group<<"] = "<<error_solution[group].l2_norm()/sn_group[group]->dof_handler.n_dofs()<<std::endl;
//}	
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>  
void SN<dim>::compute_L2_norm_error(unsigned int group)
