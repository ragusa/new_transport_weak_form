...
{	// this function reads the material data
  ...
  for(unsigned int g=0; g<n_groups; g++)
  {
	...
	for(unsigned int i_dof = 0; i_dof < dof_handler.n_dofs(); i_dof++)
	{
		...
		k_abs_vertex(i_dof) = 1; //std::fabs(nodal_data(i_dof, 1)); //*100;  //debug, change unit from 1/m to 1/cm
	}
  ...
  for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
    ...
    for(unsigned int i_dof=0; i_dof<dofs_per_cell; i_dof++)
    {
    	...
    	T4_vertex(local_dof_indices[i_dof]) = std::fabs(cos(M_PI/2.0*dof_support_point(0))*
   	  ...
   	  std::vector<double> local_k_abs(n_q_points);
   		...
   		for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
   		{
   			...
   			if(g==0)
   			  ...
   			  T4(i_cell, q_point) = std::fabs(cos(M_PI/2.0*point(0))*
    		                                  cos(M_PI/2.0*point(1))*
    		                                  cos(M_PI/2.0*point(2)));
...
class RHS : public Function<dim> // this class has the information that we need for the right-hand-side due to even parity components
{
 public :
  RHS() : Function<dim>() {};
  virtual double get_source (const Point<dim>   &p, const double sa, unsigned int group, const unsigned int  component = 0) const;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>
double RHS<dim>::get_source (const Point<dim> &p, const double sa, unsigned int group, const unsigned int /*component*/) const 
...
//  value = 4.0*sigma_Boltzmann*T4/(4.0*M_PI);
  ...
  value = sa*1.0/(4.0*M_PI)*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
...
// 	 cout<<"alpha = "<<alpha<<endl;   //debug
   ...
   double return_value = ( value/(4.0*M_PI) )*alpha;
...
//        value = Omega[moment][0]*phi/(4*M_PI)*2.0*M_PI/xlen*cos(2.0*M_PI*(p[0]-xlen/2.0)/xlen);  //sin(x)sin(y) manufacturered solution
        ...
        value = -Omega[moment][0]*1.0/(4.0*M_PI)*M_PI/2.0*sin(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2])
                -Omega[moment][1]*1.0/(4.0*M_PI)*M_PI/2.0*cos(M_PI/2.0*p[0])*sin(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2])
        			  -Omega[moment][2]*1.0/(4.0*M_PI)*M_PI/2.0*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*sin(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
...
SN_group<dim>::SN_group(const unsigned int          group,  // this is the constructor of the SN_group object, we give some values by default
    ...
    
 {
   ...
   n_Omega = (dim==2 ? 2*(1+n_moments/2)*n_moments/4
                     : 4*(1+n_moments/2)*n_moments/4);
...
{ 
 ...
 for (unsigned int moment=0; moment<n_moments/2; moment++)  //only half the shpere will be enough for 2D problem
 {
  int level = moment + 1; //latitude level index
  cout<<"Level("<<level<<"):"<<endl;
  double mu = mu_quadrature.point(n_moments - level)[0]*2.0-1.0;  //mu = cos(\theta), polar angle cosine
  ...
  for(int n_w=0; n_w<(dim==2 ? 2*level : 4*level); n_w++)
  {
   ...
   wt.push_back(2.0*mu_quadrature.weight(n_moments - level)*2.0*M_PI/(4.0*level));  //polar weights sum to 2, total weights sum to 4*pi 
   cout<<temp[0]<<" "<<temp[1]<<endl;
   cout<<"wt = "<<2.0*mu_quadrature.weight(n_moments - level)*2.0*M_PI/(4.0*level)<<endl;
  }
 }
 ...
 for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  ...
  for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
   for (unsigned int i=0; i<dofs_per_cell; ++i)
   {
    ...
    cell_rhs(i) +=(
          right_hand_side.get_source (fe_values.quadrature_point (q_point), sa_t[q_point], group) * 
          fe_values.shape_value (i, q_point) *
          fe_values.JxW (q_point));
    ...
    if (cell->at_boundary(face) )
    {
     ...
     if(parameters.boundary_conditions[side] == 2)
     {
      ...
      for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point) // we modify the matrix and the right-hand-side because of the boundary condition
      {
       for (unsigned int i=0; i<dofs_per_cell; ++i)
       { 
        cell_rhs(i) +=  ( std::fabs( Omega[m] * fe_face_values.normal_vector(q_point) ) *
            right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point), 
                                     (side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), 
                                     group, m+1, Omega, wt)  *
            fe_face_values.shape_value(i,q_point) *
            fe_face_values.JxW(q_point));
        ...
        for (unsigned int j=0; j<dofs_per_cell; j++)  
         cell_matrix(i,j) +=  ( std::fabs( Omega[m] * fe_face_values.normal_vector(q_point) ) *
              fe_face_values.shape_value(i,q_point) *
              fe_face_values.shape_value(j,q_point)*
              fe_face_values.JxW(q_point));
       }
...
//compute scalar flux using angluar flux
{
 ...
 for(unsigned int level=1; level<=material_data.get_n_moments()/2; level++)
  for(unsigned int n_w=0; n_w<(dim==2 ? 2:4)*level; n_w++)
  {
   unsigned int angle = (dim==2 ? 2:4)*(1+(level-1))*(level-1)/2+n_w;
   temp = sn_group[group]->solution_moment[angle];
   phi[group] += temp*=((dim==2?2.0:1.0)*2.0*wt[angle]);
  }
}
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template<int dim>
  ...
  for(unsigned int group=0; group<n_groups; group++)
  {
    ...
    for(unsigned int level=1; level<=material_data.get_n_moments()/2; level++)
     for(unsigned int n_w=0; n_w<(dim==2 ? 2:4)*level; n_w++)
     {
       ...
       phi_spn_recon[group] += temp*=((dim==2?2.0:1.0)*2.0*wt[angle]);
     }
  }
}
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//compute response using even parity angluar flux and adjoint boundary conditions
{
                ...
                for(unsigned int n_w=0; n_w<(dim==2?2:4)*level; n_w++)
            {
              ...
              {                    
                  leakage +=  std::fabs( Omega[angle] * fe_face_values.normal_vector(q_point) ) *
                            alpha*2.0*(face_psi_values[q_point] )*  // \psi(\Omega) += 2*psi+*(\Omega), \Omega*n >0  //check J_{spn} ?= J_{recon}
  //                               - face_psi_values_recon[q_point])*  // \psi(\Omega) += 2*psi+*(\Omega), \Omega*n >0
                  ...
                  leakage_recon +=  std::fabs( Omega[angle] * fe_face_values.normal_vector(q_point) ) *
                            alpha*2.0*( //face_psi_values[q_point]  // \psi(\Omega) += 2*psi+*(\Omega), \Omega*n >0  //check J_{spn} ?= J_{recon}
                                  face_psi_values_recon[q_point])*  // \psi(\Omega) += 2*psi+*(\Omega), \Omega*n >0
                           (dim==2?2.0:1.0)*wt[angle]*                       //integrate over all directions                                                                                                  
                           fe_face_values.JxW(q_point);        //integrate over cell volume
                  ...
                  source +=  std::fabs( Omega[angle] * fe_face_values.normal_vector(q_point) ) *
                           (- right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point), 
                             (side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), 
                             group, angle+1, Omega, wt))*  // \psi(\Omega) += - f(-\Omega), \Omega*n >0
                           (dim==2?2.0:1.0)*wt[angle]*                       //integrate over all directions                                                                                                  
                           fe_face_values.JxW(q_point);        //integrate over cell volume 
	...
	d_response[group] = leakage - leakage_recon;
  cout<<"Leakage of Interest: "<<leakage<<endl;
  cout<<"Leakage_recon of Interest: "<<leakage_recon<<endl;
  cout<<"Error of Interest: "<<d_response[group]<<endl;
...
void SN<dim>::check_conservation(unsigned int group)
{
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
	  ...
	  for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
	  {
        ...
        {
        	unsigned int angle = (dim==2?2:4)*(1+(level-1))*(level-1)/2+n_w;
		  	  source += right_hand_side.get_source (fe_values.quadrature_point (q_point), sa_t[q_point], group) *
		  	            (dim==2?2.0:1.0)*wt[angle]* 2.0*
              ...
              for(unsigned int n_w=0; n_w<(dim==2?2:4)*level; n_w++)
              {
                ...
                sink +=  abs( Omega[angle] * fe_face_values.normal_vector(q_point)  ) *
                       (2.0*face_psi_values[q_point]
                        - right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point), 
                               (side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), 
                               group, angle+1, Omega, wt) ) *  // \psi(\Omega) = 2*psi+*(\Omega) - f(-\Omega), \Omega*n >0
                       (dim==2?2.0:1.0)*wt[angle]*                       //integrate over all directions                                                                                                  
                       fe_face_values.JxW(q_point);        //integrate over cell volume
...
{ // this function makes the output for the scalar flux of the direct and the adjoint problem
	for(unsigned int group = 0; group<material_data.get_n_groups(); group++)
  {
   ...
   // data_out.add_data_vector (psi_plus_recon[1], "psi_plus_1");
    ...
    std::ostringstream d_response_filename;
    d_response_filename<<"Response_forward-"<<group<<".txt";
    std::ofstream d_Response_out(d_response_filename.str().c_str());
    d_Response_out.precision(12);
    d_Response_out<<d_response[group]<<std::endl;
    d_Response_out.close();
...
void SN<dim>::run () 
 ...
 { 
  ...
  for(unsigned int j=0; not_converged && j<=0 ;j++) // we solve the direct problem. we need to iterate over all the moments until we converge
  {
