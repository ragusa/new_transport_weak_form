...
{	// this function reads the material data
  ...
  for(unsigned int g=0; g<n_groups; g++)
  {
	...
	for(unsigned int i_dof = 0; i_dof < dof_handler.n_dofs(); i_dof++)
	{
		...
		k_abs_vertex(i_dof) = 0.01; //std::fabs(nodal_data(i_dof, 1)); //*100;  //debug, change unit from 1/m to 1/cm
	}
...
class RHS : public Function<dim> // this class has the information that we need for the right-hand-side due to even parity components
{
	public :
		...
		virtual double get_source (const Point<dim>   &p, unsigned int group, const double sa, bool diffusion, const unsigned int  component = 0) const;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>
double RHS<dim>::get_source (const Point<dim> &p, unsigned int group, const double sa, bool diffusion, const unsigned int /*component*/) const 
...
//		value = 4.0*sigma_Boltzmann*T4;
		value = (sa+ (M_PI/2.0)*(M_PI/2.0)/3.0/sa)*cos(M_PI/2.0*p[0]);  //sin(x) manufacturered solution
		value = (sa+ 3.0*(M_PI/2.0)*(M_PI/2.0)/3.0/sa)*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
	...
	else
	{
		...
		//value = 4.0*sigma_Boltzmann*T4;
		value = sa*cos(M_PI/2.0*p[0]);  //sin(x) manufacturered solution
		value = sa*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
	}	
	if(suppress)
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
    {
			...
			for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
				for (unsigned int i=0; i<dofs_per_cell; ++i)
				{
					...
					cell_rhs(i) +=( 
									 right_hand_side.get_source (fe_values.quadrature_point (q_point), group, sa_t[q_point], diffusion) *
								   fe_values.shape_value (i, q_point) *
								   fe_values.JxW (q_point));
							...
							{	
								if(!diffusion)
								{
	  							cell_rhs(i) +=  (mu[m] *
	  											...
	  											(side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), group, m+1, mu, wt)  *
	  											fe_face_values.shape_value(i,q_point) *
	  											fe_face_values.JxW(q_point));
	  						...
	  						else		//boundary condition for diffusion equation
	  						{
	  							cell_rhs(i) +=  ( 0.5*
	  											right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point),
	  											(side==8?4.0*sigma_Boltzmann*T4[q_point]:parameters.boundary_value[side]), group, m+1, mu, wt)  *
	  											fe_face_values.shape_value(i,q_point) *
	  											fe_face_values.JxW(q_point));
          ...
          fe_face_values.get_function_values(material_data.get_T4_vertex(), T4_face);
            ...
            fe_face_values.get_function_values(psi_plus_m,face_psi_values);	
              ...
              source += std::fabs( mu[m] ) *
                       (  - right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point),
  	  											(side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), group, m+1, mu, wt) ) *  // \psi(\Omega) = 2*psi+*(\Omega) - f(-\Omega), \Omega*n >0
                       wt[m]*                       //integrate over all directions                                                                                                  
                       fe_face_values.JxW(q_point);        //integrate over cell volume
            }
          }
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
	  fe_values.reinit (cell);
		...
		for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
		{
			...
			for(int m=0; m<double(material_data.get_n_moments()/2.0); m++)
		  {
		  	source += right_hand_side.get_source (fe_values.quadrature_point (q_point), group, sa_t[q_point], diffusion) *
		  	          2.0 * wt[m]*
		...
		if (boundary_exist != parameters.boundary_conditions.end()) 
		{
																												...
																												// condition we go to the if
		  {
  		  unsigned int side = cell->face(face)->boundary_indicator()-1;
              ...
              source += std::fabs( mu[m] ) *
                        right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point),
  	  											(side==8?4.0*sigma_Boltzmann*T4_face[q_point]:parameters.boundary_value[side]), group, m+1, mu, wt)  *    // \psi(\Omega) = f(\Omega), \Omega*n <0
                        wt[m]*                       //integrate over all directions                                                                                                  
                        fe_face_values.JxW(q_point);        //integrate over cell volume
            }
          }
