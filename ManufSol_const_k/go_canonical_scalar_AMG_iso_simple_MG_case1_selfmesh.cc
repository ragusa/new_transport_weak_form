...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void MaterialData::set_n_moments(unsigned int N)		//set SPn order in different level of AMG
{
	n_moments = N;
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void MaterialData::correct_scat_moments(unsigned int group)		//get the optimal corrected cross-section for current AMG level. 
...
class RHS : public Function<dim> // this class has the information that we need for the right-hand-side due to even parity components
{
	public :
		...
		virtual double get_source (const Point<dim>   &p, unsigned int group, double sigma_t, double sigma_s, bool diffusion,	std::vector<double> domain_size, const unsigned int  component = 0) const;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <int dim>
double RHS<dim>::get_source (const Point<dim> &p, unsigned int group, double sigma_t, double sigma_s, bool diffusion,	std::vector<double> domain_size, const unsigned int /*component*/) const 
...
//		value=1;  
	...
	{
		//value = phi;
		value = (sigma_t - sigma_s+ (M_PI/2.0)*(M_PI/2.0)/3.0/sigma_t)*cos(M_PI/2.0*p[0]);  //sin(x) manufacturered solution
		value = (sigma_t - sigma_s+ 3.0*(M_PI/2.0)*(M_PI/2.0)/3.0/sigma_t)*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
	}
	else
	{
		//value = phi;
		value = sigma_t*cos(M_PI/2.0*p[0]);  //sin(x) manufacturered solution
		value = sigma_t*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
	}	
	if(suppress)
...
//        value[1] = mu[m]*phi*2.0*M_PI/xlen*sin(2.0*M_PI*(p[0]-xlen/2.0)/xlen)*cos(2.0*M_PI*(p[1]-xlen/2.0)/xlen);  //sin(x)sin(y) manufacturered solution
        value[0] = -mu[m]*M_PI/2.0*sin(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);
        value[1] = -mu[m]*M_PI/2.0*cos(M_PI/2.0*p[0])*sin(M_PI/2.0*p[1])*cos(M_PI/2.0*p[2]);  //sin(x)sin(y) manufacturered solution
        value[2] = -mu[m]*M_PI/2.0*cos(M_PI/2.0*p[0])*cos(M_PI/2.0*p[1])*sin(M_PI/2.0*p[2]);
...
class SPN_group
{
	public :
		SPN_group (const unsigned int        		m,		//m is the direction index
					MaterialData       		&material_data_new,
          MaterialData					&material_data_old,
  ...
  FEValues<dim> fe_values (fe, quadrature_formula, 
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
    {
		...
		if (material_data.get_n_moments()>1)
		{
			...
			diffusion_coefficient = 1.0/(3.0*(st-ss1));   // we create the diffusion coefficient
		}
...
class SPN // here we define the SPN class which contains the Parameters class
{
	...
	private :
  	...
  	void compute_angular_quadrature(unsigned int n_moments, 
		...
		const MaterialData      				material_data;
		MaterialData										material_data_new;
		MaterialData										material_data_old;  //auxiliary material_data used in Angular Multi-Grid
...
Triangulation<3> SPN<3>::initial_grid()
{
  ...
  if (parameters.with_assemblies)
	...
	for (unsigned int j=0; j<parameters.z_assembly_subdivisions[i]; j++,mz++)
      ...
      const Point<3> bottom_left = Point<3>(-1,-1,-1);
      const Point<3> upper_right = Point<3> (xlen-1, ylen-1, t_height-1);
...
//	spn_group[group]->system_matrix.reinit (spn_group[group]->sparsity_pattern); // now the matrix receive her pattern
  ...
  FEValues<dim> fe_values (fe, quadrature_formula, 
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
    {
			...
			double diffusion_coefficient = pow(mu[m],2)/st;   // we create the diffusion coefficient
			if(diffusion)
				 diffusion_coefficient = 1.0/(3.0*st);   // we create the diffusion coefficient
			double ss0 = material_data.get_moment_XS(cell->material_id(), group, 0); //get 0th-moment of the scattering cross-section
			...
			for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
				for (unsigned int i=0; i<dofs_per_cell; ++i)
				{
					...
					cell_rhs(i) +=( 
									right_hand_side.get_source (fe_values.quadrature_point (q_point), group, st_origin, ss0, diffusion, domain_size) *
								   fe_values.shape_value (i, q_point) *
								   fe_values.JxW (q_point));
				}	  
							...
							{	
								if(!diffusion)
								{
	  							cell_rhs(i) +=  (mu[m] *
	  											...
	  											parameters.boundary_value[side], group, m+1, mu, wt)  *
	  											fe_face_values.shape_value(i,q_point) *
	  											fe_face_values.JxW(q_point));
	...
	{		
		...
		for (unsigned int i=0; cell!=endc; ++cell, ++i) 
		{
			...
			for (unsigned int j=0; j<dofs_per_cell; ++j)  //j = local dof index
			{
				...
				for(unsigned int m=0; m<n_moments/2; m++)
				{
					temp += -mu[m]/st*spn_group[group]->solution_moment[m](local_dof_indices[j])*2.0*Pnm[moment][m]*wt[m];  //grad(psi^+) term in odd-parity equations
					if(!suppress_source)
						temp += 1.0/st*q_minus[group][m](i*dofs_per_cell+j)*2.0*Pnm[moment][m]*wt[m];  //odd parity external source q^- term, due to Legendre orthogonality
				}
				temp += ss/st*phi_odd_old[group][moment](i*dofs_per_cell+j); //only the "moment"-th moment remains, due to Legendre orthogonality
...
void SPN<dim>::compute_Leakage(unsigned int group)
{
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
		...
		if (boundary_exist != parameters.boundary_conditions.end()) 
		{
            ...
            sink +=  abs( mu[m] ) *
                     (2.0*face_psi_values[q_point] ) *  // \psi(\Omega) = 2*psi+*(\Omega) - f(-\Omega), \Omega*n >0
            ...
            source += abs( mu[m] ) *
                     (  - right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point),
	  											parameters.boundary_value[side], group, m+1, mu, wt) ) *  // \psi(\Omega) = 2*psi+*(\Omega) - f(-\Omega), \Omega*n >0
                     wt[m]*                       //integrate over all directions                                                                                                  
                     fe_face_values.JxW(q_point);        //integrate over cell volume
          }
        }
  ...
  cout<<"Half-Range-Current : "<<response<<endl;
	...
	for (unsigned int i_cell=0; cell!=endc; ++cell, ++i_cell) 
  {
	  fe_values.reinit (cell);
		...
		for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
		{
			...
			for(int m=0; m<double(material_data.get_n_moments()/2.0); m++)
		  {
		  	source += right_hand_side.get_source (fe_values.quadrature_point (q_point), group, st_origin, ss0, diffusion, domain_size) *
		  	          2 * wt[m]*
		...
		if (boundary_exist != parameters.boundary_conditions.end()) 
		{
            ...
            source += abs( mu[m] ) *
                      right_hand_side.get_Jinc (fe_face_values.quadrature_point (q_point),
	  											parameters.boundary_value[side], group, m+1, mu, wt)  *    // \psi(\Omega) = f(\Omega), \Omega*n <0
                      wt[m]*                       //integrate over all directions                                                                                                  
                      fe_face_values.JxW(q_point);        //integrate over cell volume
          }
        }
